...
Credentials are stored only in app-private SharedPreferences. No account is embedded in the APK. When credentials are absent or the login fails, the normal player error state tells the user that Markíza login details are required.
